...
int main(int argc, char **argv){
  ...
  while (1){
	...
	//fsp: A=65=up, B=66=down, C=67=right, D=68=left
	// ^[[A^[[B^[[C^[[D^[^C

      //Move left motor
	//fsp: left arrow is pressed, i.e. character sequence is ^[[D = 27,91,68
	...
	//fsp: right arrow is pressed, i.e. string is ^[[D = 27,91,67
      ...
      //Move right motor
	//fsp: up arrow is pressed, i.e. string is ^[[D = 27,91,65
	...
	//fsp: down arrow is pressed, i.e. string is ^[[D = 27,91,66
      ...
      //Pen UP/DOWN
      //fsp: check what code is used in OS X (cmd + up / cmd + down) or change key!
      ...
      if(KeyHit == 'm'){
        //MoveLength *= 10;
        //if(MoveLength == 10000){
	//fsp: changed because sometimes MoveLength needs to be large, e.g. for large wall installations
        MoveLength *= 10;
        if(MoveLength > 10000){
          MoveLength = 1;
        }
